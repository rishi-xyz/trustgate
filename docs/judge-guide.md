...
**Endpoint:** `<PUBLIC_URL>` (MCP over HTTPS, no login)
**Enclave measurement (PCR0), published:** `<PCR0>`
...
```bash
claude mcp add --transport http trustgate <PUBLIC_URL>
```
...
b,13
c,5000
and call verify_receipt with the receipt_id and expected_measurement set to <PCR0>. Tell me exactly which checks passed and what this receipt does and does not prove.
```

Expected: mode `nitro`, sums a=22, b=24, c=5000, five checks passing (including the measurement), and an honest statement of the limits.

**opencode** (`opencode.json`)

```json
{ "mcp": { "trustgate": { "type": "remote", "url": "<PUBLIC_URL>", "enabled": true } } }
```
...
trustgate exec -url <PUBLIC_URL> -workload csv-stats -input data.csv -args amount,supplier -out receipt.json
trustgate verify receipt.json -measurement <PCR0>
trustgate replay receipt.json -measurement <PCR0> -wasm csv-stats.wasm -stdin data.csv
```
